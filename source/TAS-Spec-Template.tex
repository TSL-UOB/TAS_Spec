...
In this paper, we classify the AS domains based on two criteria: the number of autonomous agents (single or multiple), and whether there are humans involved in the interaction \cite{Schneiders2022}. 
Accordingly, typical domains of AS can take one of four forms: (i) a single autonomous agent (e.g. automated driving, UAV); (ii) a group of autonomous agents (e.g. swarms); (iii) an autonomous agent with a human (e.g. AI in healthcare, human-robot interaction); and (iv) a group of autonomous agents with humans (e.g. emergency situations and disaster relief). 
First and foremost, this exercise is not intended to be exhaustive.
Due to the space considerations, we now discuss specification challenges of a representative set of domains which are being investigated by the different Nodes of TAS (see Table.~\ref{AS-challenges}). 
%\footnotesize
%\begin{center}
%	\begin{table*}
